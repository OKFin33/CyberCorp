...
For an existing Corp adopting 0.1.0a4, compare package `docs/corp/` and all three `.agents/skills/` methods with project-owned versions. Include the new milestone-delivery and communication targets; point communication at the existing Owner card or adapt the neutral one. Templates link the actual method/card instead of copying preferences. No new installer input is needed.
