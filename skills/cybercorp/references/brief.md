...
For an existing Corp adopting a newer package, compare package `docs/corp/`, `.agents/skills/` and `.agents/corp/` with project-owned versions. Carry the current entry and `mechanics.md` together with the portable `check.py`, and connect checks to the project's actual command/CI entry per the `mechanics.md` section on completion resting on evidence that is not self-reported. Preserve existing Owner cards, explicit review promises and installation provenance; reinstallation still refuses changed packages. No new installer input is needed.

Carry relevant method changes through the target's normal change process. A project still holding the pre-2026-09-12 layout has eleven separate method documents under `docs/corp/`; replace them with the current entry plus `mechanics.md`, keeping its own Owner card. The rules formerly spread across `development-loop`, `claim-protocol`, `spec-authoring`, `spec-protocol`, `verification`, `milestone-delivery`, `coordination`, `communication`, `damaged-history` and `bootstrap` are now defined once under the eight sub-problems in `mechanics.md`, each named by the rule itself rather than by a code; the old ordinary-PR default review trigger is replaced by the section on completion resting on evidence that is not self-reported, where stage review opens on machine-checkable closing conditions. Preserve accepted explicit review promises, task Spec pins, project edits and initial install provenance. The adopting change itself follows its pre-change agreement, including independent review. Check file adoption, effective agreement and actual instance consumption separately. Reinstallation continues refusing changed packages; it is not a migration command.
