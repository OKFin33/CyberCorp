...
- **S7 的接管条件改为可执行的观察记录。** 原文以"超过阈值未变化"为条件却不设默认值，使 S7 自称消除的猜测在项目未声明阈值时依然存在。现改为：核对最后变化时间、未关闭的 PR 与剩余工作，把观察记录在对象上，然后自我指派；项目可声明阈值作为跳过该记录的捷径。该改动依赖 S7 原有的"原持有者回来后必须停止"义务来限制过早接管的代价。
