...
**当前状态：早期。** 创建包候选为 `0.1.0a10`，尚未发布正式版本。哪些能力有证据、哪些还没有，逐项记录在[验证状态](docs/verification-status.md)——请在依赖任何能力之前先看那一份。

**前提。** 目标必须是一个**托管在 GitHub 上的仓库**，且已完成建立（见下）。这套机制的每一个载体都是 GitHub 原生对象——Issue 持有工作单元、PR 与 checks 持有完成证据、Milestone 持有阶段、`assignee` 与评论持有占用。没有托管仓库不是降级可用，而是这些载体都不存在。**其他托管平台尚未适配。**
...
第一条在同一克隆上从固定远端 SHA 加一个隔离工作树，原工作树保持原样，并在输出里说明它给原库添了什么、这个现场怎么结束；中间两条读取真实 GitHub 工作。`7` 换成实际 Issue。上下文读取使用远端默认分支，依赖目标项目已共享其 Corp 入口及原生工作。

最后一条仅做本地 Corp 结构和实际 Git 差量检查，不联网或运行项目业务测试。检查结果分别报告本地结构与尚未核验的共享路由；创建 Agent 按项目需要将它接入现有检查命令与 CI。具体版本、失败与复现方法见生成项目 `docs/corp/mechanics.md` 中「完成以非自证的证据为准」一节。
