...
当前创建包候选为 `0.1.0a4`；仓库提供两个可独立携带的能力包：
...
`0.1.0a4` 按阶段实际缺口选择 plan、work 或 review；阶段交付和 Owner 沟通方法按需读取，项目可替换自己的沟通卡。普通工作仍核验有效约定与执行占用，并保留任务内修复和局部暂停边界。同一最小生成项目的普通 work 完整方法冷读约 2.75–2.78k tokens，[计量路径与行为证据](docs/evidence/issue-3/README.md)可复核；宿主、业务材料和原生任务历史另计，数字不是实际运行账单。
