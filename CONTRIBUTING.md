...
| `CHANGELOG.md`、`docs/verification-status.md` | 变更与验证程度 | 维护者 |
| `docs/decisions/` | 规则的来源与被否决的替代方案 | 改机制层者 |
...
**决策记录不是可选的参考资料。** 机制层有一条规则把理由排除在外（「规则写这里，理由归记录」），那条规则要求另一处接住它——就是 `docs/decisions/`。使用产品的人不必读它；改机制层的人必读也必写。检查器对缺失只给不阻塞的提示而非硬门禁，因为硬门禁会教人写空壳来通过，而为通过而写的记录会编造理由。
...
**留白要带默认行为。** 规则允许留白——有些内容只能在使用中形成，写死反而是伪造经验。但留白处必须有可执行的默认行为，否则执行者会卡在那里猜。判断方法：设想一个什么都没声明的项目，执行者读到这条规则时知道该做什么吗？「不可逆动作清单」留白但有兜底（清单之外一律自主），所以成立；「接管阈值」曾经留白且没有兜底，执行者不知道该等还是该接管，那是缺陷。

**不要写自己检验不了的声明。** 每条规则都要说明它消除了哪个猜测，而那句话本身是一个声明。写完后设想最不利的合法状态——项目什么都没声明、多个实例共用一个账号、上一个实例无声消失——那个猜测真的被消除了吗？同理适用于工具的字段名：名字不得强于它实际计算的东西。

**方法文件（`.agents/skills/*/SKILL.md`）只给顺序，不给规则。** 每一步一句动作，后面标它服务的规则名，规则内容一律不复述——那会产生第二个定义处，两边迟早分叉。不要把它写成详尽的菜谱：现在的模型能处理歧义，过度具体的分步反而会限制它。front matter 的 `description` 要短，并且说清**何时**适用而不是**涉及什么**——"涉及数据库时使用"会让它在任何碰到数据库的场合被加载，"新增或修改迁移时使用"才准确。它会被不同模型读到，为某个模型写的强硬措辞可能过度约束另一个。
